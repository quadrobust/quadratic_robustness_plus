...
IMG_PATH = Path("tests/dog.jpg")      # wgraj cokolwiek z ImageNet‑val
...
def test_quadratic_warp_sanity(tmp_path):
    ...
    warped = quadratic_warp(x, form_id=1, eps_aff=0.1, eps_trans=0.1)
    ...
    out_path = Path(__file__).parent / "dog_warp.png"
    save_image(warped.cpu(), out_path)
